...
#Then creates a base plot with 4 graphs in the file for Datetime and Sub Meterings, Voltage
...
#Convert Global active, Global reactive power and voltage to numeric format
global_active_power <- as.numeric(file_data$Global_active_power)
global_reactive_power <- as.numeric(file_data$Global_reactive_power)
voltage   <- as.numeric(file_data$Voltage)

#Create a png file to store the Graph
png("plot4.png", width = 480, height = 480)

#Initialize Par for displaying 4 graph plots
par(mfrow = c(2,2))

#First plot - Date time and Global active Power
plot(Datetime, global_active_power, type = "l", xlab = "", ylab = "Global Active Power")

#Second plot - Date time and Voltage
...
#Fourth plot - Date time and Global reactive power
